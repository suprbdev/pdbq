-- field: allUsers
SELECT jsonb_build_object('nodes', coalesce(jsonb_agg(__s_2.ndata ORDER BY __s_2.__rn ASC) FILTER (WHERE __s_2.__rn <= $3), '[]'::jsonb), 'edges', coalesce(jsonb_agg(jsonb_build_object('cursor', __s_2.__cursor, 'node', __s_2.edata) ORDER BY __s_2.__rn ASC) FILTER (WHERE __s_2.__rn <= $3), '[]'::jsonb), 'totalCount', (SELECT count(*) FROM "public"."users" AS __c_3)::int, 'pageInfo', jsonb_build_object('hasNextPage', (count(*) > $3), 'hasPreviousPage', true, 'startCursor', (jsonb_agg(__s_2.__cursor ORDER BY __s_2.__rn ASC) FILTER (WHERE __s_2.__rn <= $3))->0, 'endCursor', (jsonb_agg(__s_2.__cursor ORDER BY __s_2.__rn ASC) FILTER (WHERE __s_2.__rn <= $3))->-1)) AS data
FROM (
